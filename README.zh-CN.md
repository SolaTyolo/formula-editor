...
- **主题定制**：内置 CSS（BEM + CSS 变量），支持 `theme` 与 `classNames` 覆盖（参考 amis InputFormula 模式）
...
采用 amis **InputFormula** 同款模式：BEM 类名（`.fel-*`）+ 根节点 CSS 变量。
...
弹层、悬停预览等没有 `theme` prop 的场景，可用自定义 class 设置相同变量（见 `example/src/editorTheme.css`）。
